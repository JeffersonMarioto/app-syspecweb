...
addForm.addEventListener("submit", async (e) => {
  ...
  if (addForm.checkValidity() === false) {
    ...
    const data = await fetch("php/action_animais.php", {
      method: "POST",
      body: formData,
    });
    const response = await data.text();
    showAlert.innerHTML = response;
    document.getElementById("add-user-btn").value = "Add Animal";
    addForm.reset();
...
const fetchAllUsers = async () => {
  const data = await fetch("php/action_animais.php?read=1", {
    method: "GET",
  });
  const response = await data.text();
...
const editUser = async (id) => {
  const data = await fetch(`php/action_animais.php?edit=1&id=${id}`, {
    method: "GET",
  });
...
const deleteUser = async (id) => {
  const data = await fetch(`php/action_animais.php?delete=1&id=${id}`, {
    method: "GET",
  });
  const response = await data.text();
  showAlert.innerHTML = response;
